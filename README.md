...
| batch_size_bytes           | False    | None                          | Maximum uncompressed batch size in bytes before flushing, regardless of row count. Useful for capping memory usage on containers ingesting wide or large records.                                                                                                                                |
| batch_timeout_minutes      | False    | 5.0                           | Maximum time in minutes before a batch is flushed, regardless of batch size. Increase this when ingesting large sources where batches fill slowly.                                                                                                                                               |
